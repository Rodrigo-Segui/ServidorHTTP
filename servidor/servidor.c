...
//         
    ...
    printf("\n\n  MT - Lidos: %d \n",total);
    printf("\n\n -----AGUARDANDO NOVA CONEXAO COM CLIENTE------");
...
void trataMensagem (char *mensagem,FILE *sp,int sock_c) 
{
  ...
  if(!strcmp(mensagem,"GET\0")) 
  {
    ...
    if(!strcmp(protocolo,"HTTP/1.0\0")) 
    {
...
int main()
{
  ...
  printf("-----Servidor HTTP - Redes de Computadores------\n\n");
  /* Variáveis */
  int socket_c, cliente_s[10];
  int PORTA;
  int TAXA_MAX;
  int TAM_CLI;
  ...
  MSG_REC = (char*)malloc(MSG_TAM*sizeof(char));
    ...
    while (1) {
      ...
      cliente_s[num] = accept(socket_c, (struct sockaddr*)&cliente, &TAM_CLI);
      printf("RECEBEU CONEXAO CLIENTE: %i\n", num);
      FILE *fp = fdopen(cliente_s[num],"r+");
      printf("MENSAGEM SERVIDOR - Cliente Conectado : %d \n",cliente_s[num]);
      printf("---------------------- Aguardando Requisicao ---------------------- \n");
      //do {
        fgets(MSG_REC,MSG_TAM,fp);
	      trataMensagem(MSG_REC, fp, cliente_s[num]);
